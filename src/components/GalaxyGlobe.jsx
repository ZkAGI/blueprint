...
function YearRail({
  ...
  // ---------- MOBILE: vertical timeline with auto-open on scroll ----------
const Mobile = () => {
  const listRef = React.useRef(null);
  const itemRefs = React.useRef([]);

  // Auto-expand the most visible item while scrolling
  React.useEffect(() => {
    if (!listRef.current) return;

    // Bias the "center" so the item near the middle of the screen wins
    const observer = new IntersectionObserver(
      (entries) => {
        // Pick the intersecting entry with the highest visibility
        let best = null;
      ...
      {
        root: null,
        // top margin negative raises the "hot zone" a bit above center,
        // bottom margin pushes it down so items near middle dominate
  ...
  return (
    <div className="md:hidden relative">
      ...
      <ul
        ref={listRef}
        className="space-y-4 pl-10 snap-y snap-mandatory"
      >
        {years.map((y, i) => {
